...
class BertSentimentClassifier(torch.nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        pooler_output = output_dict['pooler_output']
        logits = self.classfier(pooler_output)
        return logits
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm(train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE):
            ...
            optimizer.zero_grad() # 每一个batch开始时积累的梯度清零
            logits = model(b_ids, b_mask)
            loss = F.cross_entropy(logits, b_labels.view(-1), reduction='sum') / args.batch_size # 'sum': 返回每个预测值与label的cross_entropy之和
...
def get_args():
    ...
    parser.add_argument("--batch_size", help='sst: 64, cfimdb: 8 can fit a 12GB GPU', type=int, default=4) # 可惜我是6GB GPU
