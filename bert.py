...
class BertSelfAttention(nn.Module):
  ...
  def transform(self, x, linear_layer):
    ...
    proj = linear_layer(x) # [bs, seq_len, hidden_state] -> [bs, seq_len, all_head_size]
  ...
  def attention(self, key, query, value, attention_mask):
    """
    key, query, value: [bs, num_attention_heads, seq_len, attention_head_size] bs*h*n*d/h
    attention_mask: [bs, 1, 1, seq_len] bs*1*1*n
    """
    # each attention is calculated following eq (1) of https://arxiv.org/pdf/1706.03762.pdf
    # attention scores are calculated by multiply query and key 
    # and get back a score matrix S of [bs, num_attention_heads, seq_len, seq_len] # bs: batch size
    ...
    bs, h, n, ahs = key.size()
    S = torch.matmul(query, torch.transpose(key, -1, -2)) / ahs ** 0.5 # bs*h*n*n
    masked_S = S + attention_mask
    weight = F.softmax(masked_S, -1) # bs*h*n*n
    # multiply the attention scores to the value and get back V'
    scores = torch.matmul(weight, value) # bs*h*n*d/h
    scores = torch.transpose(scores, 1, 2).contiguous() # bs*n*h*d/h
    
    # V_ = torch.matmul(weight, value).transpose(1, 2).contiguous() # contiguous()保证tensor是连续储存的
    
    # next, we need to concat multi-heads and recover the original shape [bs, seq_len, num_attention_heads * attention_head_size = hidden_size]
    scores = scores.view(bs, n, -1) # -1 自动计算剩下维度 bs*n*d
    return scores
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    self_attention_output = self.self_attention(hidden_states, attention_mask) # 相当于在调用BertSelfAttention class 的forward方法
    add_norm_output = self.add_norm(hidden_states,
                                      self_attention_output, self.attention_dense,
                                      self.attention_dropout, self.attention_layer_norm)
    feed_forward_output = self.interm_af(self.interm_dense(add_norm_output))
    layer_output = self.add_norm(add_norm_output, feed_forward_output, 
...
class BertModel(BertPreTrainedModel):
  ...
  def forward(self, input_ids, attention_mask):
    ...
    # sequence_output[:, 0]： [batch_size, hidden_size]，返回bs中每个seq中第一个embedding的hidden state，也就是CLS的embedding
    first_tk = sequence_output[:, 0]
    first_tk = self.pooler_dense(first_tk)
    first_tk = self.pooler_af(first_tk) # af = activate function

    return {'last_hidden_state': sequence_output, 'pooler_output': first_tk} # pooler_output: seq的[CLS] token表示
